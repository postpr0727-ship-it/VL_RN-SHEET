...
export default async function handler(
  ...
  try {
    ...
    return res.status(500).json({
      status: 'error',
      firebase: 'crash',
      message: 'Serverless Function에서 예기치 않은 오류가 발생했습니다.',
      error: globalError instanceof Error ? globalError.message : '알 수 없는 오류',
      details: globalError instanceof Error ? globalError.stack : undefined,
    });
  }
}
