...
### 4.3 환경 변수 추가 또는 업데이트

#### 상황 A: 환경 변수가 없는 경우 (처음 설정)
...
#### 상황 B: 환경 변수가 이미 있는 경우 (업데이트)

⚠️ **오류 메시지**: "A variable with the name `FIREBASE_SERVICE_ACCOUNT` already exists..."
...
2. 항목의 오른쪽 끝에 있는 **연필 아이콘 ✏️** 또는 **"Edit"** 버튼 클릭
3. **Value** 필드의 기존 내용을 **모두 선택** (Ctrl+A / Cmd+A) 후 **삭제**
4. Firebase 서비스 계정 키 JSON 파일 **전체 내용 붙여넣기**
   - 중괄호 `{` `}` 포함해서 전체 복사
5. **Environment** 확인: Production, Preview, Development 모두 체크되어 있는지 확인
6. **"Save"** 또는 **"저장"** 버튼 클릭

**방법 2: 삭제 후 재생성**

1. 환경 변수 목록에서 **`FIREBASE_SERVICE_ACCOUNT`** 찾기
2. 항목의 오른쪽 끝에 있는 **휴지통 아이콘 🗑️** 또는 **"Delete"** 버튼 클릭
3. 삭제 확인 대화상자에서 **"Delete"** 클릭
4. **"Add New"** 버튼 클릭
5. 새로운 환경 변수 추가 (위의 "상황 A" 참고)
